...
public class GitlabAPIWrapper {
	...
	public Group getGroup(String path) {
		return run(() -> api.getGroupApi().getGroup(path));
	}

	public Group createGroup(GroupParams groupParams) {
		return runOnlyIfDryRun(() -> api.getGroupApi().createGroup(groupParams));
	}
	...
	public Member addGroupMember(Group group, Long userId, AccessLevel accessLevel) {
		return runOnlyIfDryRun(() -> api.getGroupApi().addMember(group, userId, accessLevel));
	}
	...
	private <T> T runOnlyIfDryRun(Callable<T> callable) {
		if (!GitSync.isDryRun()) {
			return run(callable);
		}
		return null;
